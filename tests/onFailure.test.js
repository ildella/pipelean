...
import {series, filter, safeScan, failFast, failLate, collect, skip} from '$src/functional'
...
test('onFailure called for failFast with {item, error}', async () => {
  ...
  const fn = async item => {
    if (item === 2) throw new Error(`Error at ${item}`)
    return item * 2
  }
...
test('onFailure called for failLate with true', async () => {
  ...
  const fn = async item => {
    if (item === 2 || item === 4) throw new Error(`Error at ${item}`)
    return item * 2
  }
...
test('onFailure works with filter', async () => {
  ...
  const predicate = async item => {
    if (item === 3) throw new Error(`Error at ${item}`)
    return item % 2 === 0
  }
...
test('onFailure works with safeScan', async () => {
  const onFailureCalls = []
  const items = [1, 2, 3]
