...
import {
  throw_, series, scan, filter,
} from '$src/functional'

test('throw_ is a frozen object with name "throw"', () => {
  expect(throw_).toEqual({name: 'throw'})
  expect(Object.isFrozen(throw_)).toBe(true)
})
...
test('throw strategy throws on first error in series', async () => {
  ...
  await expect(
    series(items, fn, {strategy: throw_}),
...
test('throw strategy returns normal shape on success in series', async () => {
  ...
  const result = await series(items, fn, {strategy: throw_})
...
test('throw strategy throws on first error in scan', async () => {
  ...
  await expect(
    scan(items, fn, 0, {strategy: throw_}),
  ).rejects.toThrow('Error at 2')
})
...
test('throw strategy throws on first error in filter', async () => {
  ...
  await expect(
    filter(items, predicate, {strategy: throw_}),
  ).rejects.toThrow('Error at 2')
})
...
test('throw strategy returns normal shape on success in filter', async () => {
  ...
  const result = await filter(items, predicate, {strategy: throw_})
